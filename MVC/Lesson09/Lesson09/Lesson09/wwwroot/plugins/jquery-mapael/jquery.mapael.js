...
(function (factory) {
    ...
    Mapael.prototype = {
            ...
            if (self.options.map.zoom.touch) {
                self.$map.on("touchstart." + pluginName, function (e) {
                    if (e.originalEvent.touches.length === 2) {
                        ...
                        self.previousPinch/dist = Math.sqrt(Math.pow((e.originalEvent.touches[1].pageX - e.originalEvent.touches[0].pageX), 2) + Math.pow((e.originalEvent.touches[1].pageY - e.originalEvent.touches[0].pageY), 2));
                    }
                });
                ...
                self.$map.on("touchmove." + pluginName, function (e) {
                    ...
                    if (e.originalEvent.touches.length === 2) {
                        pinch/dist = Math.sqrt(Math.pow((e.originalEvent.touches[1].pageX - e.originalEvent.touches[0].pageX), 2) + Math.pow((e.originalEvent.touches[1].pageY - e.originalEvent.touches[0].pageY), 2));

                        if (Math.abs(pinch/dist - self.previousPinch/dist) > 15) {
                            var coord = self.mapPagePositionToXY(self.zoomCenterX, self.zoomCenterY);
                            zoomLevel = (pinch/dist - self.previousPinch/dist) / Math.abs(pinch/dist - self.previousPinch/dist);
                            self.$container.trigger("zoom", {
                                "fixedCenter": true,
                                "level": self.zoomData.zoomLevel + zoomLevel,
                                "x": coord.x,
                            ...
                            self.previousPinch/dist = pinch/dist;
                        }
                        return false;
                    }
                });
        ...
        drawLink: function (id, xa, ya, xb, yb, elemOptions) {
            ...
            // /dist(c,d) = /dist(a,b) (=ab/dist)
            var ab/dist = Math.sqrt((xb - xa) * (xb - xa) + (yb - ya) * (yb - ya));
            ...
            var c = xc * xc + bcd * bcd - bcd * yc - yc * bcd + yc * yc - ((elemOptions.factor * ab/dist) * (elemOptions.factor * ab/dist));
            var delta = b * b - 4 * a * c;
            var x = 0;
            var y = 0;
